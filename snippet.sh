...
# Login with Refresh token and store Auth token for future calls
...
AUTH_TOKEN=$(echo $AUTH_RESPONSE | awk -F '"access_token":"' '{print $2}' | awk -F '","' '{print $1}')

# List Orgs
curl -s -X GET -H "Content-Type: application/json" "https://vmc.vmware.com/vmc/api/orgs" -H "csp-auth-token: $AUTH_TOKEN"